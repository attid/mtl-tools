...
1.  **Переменная окружения `DATABASE_URL`**: Если эта переменная установлена, ее значение будет использовано в первую очередь.
2.  **`sqlalchemy.url` в `alembic.ini`**: Если `DATABASE_URL` не установлена, будет использовано значение из `alembic.ini`.

**Рекомендуется использовать `DATABASE_URL` для production-окружений и `alembic.ini` для локальной разработки.**
