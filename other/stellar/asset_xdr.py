    ...
    for account in accounts:
        ...
        for balance in balances:
            if balance["asset_type"][0:15] == "credit_alphanum":
                if (
                    balance["asset_code"] == MTLAssets.damircoin_asset.code
                    and balance["asset_issuer"] == MTLAssets.damircoin_asset.issuer
                ):
    ...
    for account in accounts_list:
        if account[2] > 0.0001:
            transaction.append_payment_op(
                destination=account[0], asset=MTLAssets.eurmtl_asset, amount=str(round(account[2], 7))
            )
...
async def get_agora_xdr():
    ...
    for account in accounts:
        ...
        for balance in balances:
            if balance["asset_type"][0:15] == "credit_alphanum":
                if (
                    balance["asset_code"] == MTLAssets.agora_asset.code
                    and balance["asset_issuer"] == MTLAssets.agora_asset.issuer
                ):
                    token_balance = balance["balance"]
                    token_balance = int(token_balance[0 : token_balance.find(".")])
        accounts_list.append([account["account_id"], token_balance, 0])
...
async def get_toc_xdr(div_sum: int):
    ...
    for account in accounts:
        ...
        for balance in balances:
            if balance["asset_type"][0:15] == "credit_alphanum":
                if (
                    balance["asset_code"] == MTLAssets.toc_asset.code
                    and balance["asset_issuer"] == MTLAssets.toc_asset.issuer
                ):
                    token_balance = balance["balance"]
                    token_balance = int(token_balance[0 : token_balance.find(".")])
...
async def get_btcmtl_xdr(btc_sum, address: str, memo=None):
    ...
    transaction.set_timeout(60 * 60 * 24 * 7)
    transaction.append_payment_op(
        destination=MTLAddresses.public_btc_guards, asset=MTLAssets.btcdebt_asset, amount=btc_sum
    )
...
async def get_chicago_xdr():
    ...
    for transaction in transactions:
        if (
            ...
            if transaction["to"] != transaction["from"] and transaction["to"] == stellar_address:
                accounts_dict[transaction["from"]] = float(transaction["amount"]) + accounts_dict.get(
                    transaction["from"], 0
                )
    ...
    for account in accounts_dict:
        ...
        transaction.append_payment_op(
            destination=account, asset=MTLAssets.eurmtl_asset, amount=str(round(cashback_sum, 7))
        )
    transaction = transaction.build()
    xdr = transaction.to_xdr()
