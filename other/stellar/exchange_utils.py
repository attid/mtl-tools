...
from stellar_sdk import Asset, Network, TransactionBuilder, Price
...
async def build_cancel_offers_xdr(
    public_key: str,
...
async def stellar_remove_orders(public_key: str, xdr: Optional[str] = None) -> Optional[str]:
    """
...
async def stellar_stop_all_exchange():
    """
    ...
    for bot in EXCHANGE_BOTS:
        xdr = await stellar_remove_orders(bot, xdr)

    if xdr:
        await stellar_sync_submit(stellar_sign(xdr, config.private_sign.get_secret_value()))
