        ...
        for member_id in users_to_remove_ids:
            ...
            if need_remove:
                try:
                    ...
                    alerts.append(
                        f"Error removing user @{username} (ID: {member_id}) from chat {chat['TITLE']}: {str(e)}"
                    )
            ...
            if admin.user_id not in [c["TELEGRAM_ID"] for c in councils]:
                try:
                    await bot.promote_chat_member(
                        chat_id=chat["TELEGRAM_ID"], user_id=admin.user_id, is_anonymous=False
                    )
                    alerts.append(
                        f"Revoked admin rights from non-council member "
                        f"@{admin.username} (ID: {admin.user_id}) in chat: {chat['TITLE']}"
                    )
                    await asyncio.sleep(1)
                except Exception as e:
                    alerts.append(
                        f"Failed to revoke admin rights from non-council member "
            ...
            if council["TELEGRAM_ID"] not in admin_ids:
                try:
                    if chat["NEED_SET_COUNCIL_TITLE"] and council["TELEGRAM_ID"] in member_ids:
                        await bot.promote_chat_member(
                            chat_id=chat["TELEGRAM_ID"], user_id=council["TELEGRAM_ID"], can_pin_messages=True
                        )
                        ...
                        alerts.append(
                            f"Made council member @{council['USERNAME']} "
                            f"(ID: {council['TELEGRAM_ID']}) admin in chat: {chat['TITLE']}"
                        )
                        await asyncio.sleep(2)
                except Exception as e:
                    alerts.append(
                        f"Failed to make council member @{council['USERNAME']} "
