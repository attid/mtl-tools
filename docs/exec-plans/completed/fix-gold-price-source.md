...
## План изменений
1. [x] Заменить блок `# aum` (строки 118-126) в `scripts/update_report.py`:
       suissegold HTML-парсинг → Kitco CSV API, `mid_price * 10`.
2. [x] Добавить защиту: try/except + logger.warning при ошибке парсинга,
       чтобы скрипт не падал если Kitco тоже временно недоступен.
3. [x] Проверка: `just lint`, `just format`, `just types` на изменённый файл.
