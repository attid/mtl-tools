...
@pytest.mark.asyncio
async def test_startup_sends_message_to_admin(mock_server, dp):
    """
    ...
    # Mock external dependencies used in on_startup
    with patch("start.work_with_support", new_callable=AsyncMock), \
         patch("start.pyro_start", new_callable=AsyncMock):
