...
async def test_start_command(mock_telegram, router_app_context):
    ...
    await dp.feed_update(router_app_context.bot, create_message_update(123, "/start"))
    ...
    next(
        (
            ...
            if r["method"] == "sendMessage" and "/start" not in r["data"].get("text", "")
        ),
        None,
    )
...
async def test_links_command(mock_telegram, router_app_context):
    ...
    await dp.feed_update(router_app_context.bot, create_message_update(123, "/links"))
...
async def test_emoji_command_no_args(mock_telegram, router_app_context):
    ...
    await dp.feed_update(router_app_context.bot, create_message_update(123, "/emoji"))
...
async def test_emoji_command_reaction(mock_telegram, router_app_context):
    ...
    await dp.feed_update(router_app_context.bot, create_message_update(123, "/emoji https://t.me/123/456 👀"))
...
async def test_save_command_admin(mock_telegram, router_app_context):
    ...
    await dp.feed_update(router_app_context.bot, create_message_update(84131737, "/save"))
