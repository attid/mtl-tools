...
def fire_mtl(account, amount):
    stellar_transaction = TransactionBuilder(source_account=account,
                                             network_passphrase=Network.PUBLIC_NETWORK_PASSPHRASE,
                                             base_fee=base_fee)
    stellar_transaction.append_payment_op(destination=mtl_asset.issuer, asset=mtl_asset, amount=str(amount))
    stellar_transaction = stellar_transaction.build()
    stellar_transaction.sign(private_sign)
    xdr = stellar_transaction.to_xdr()
    logger.info(f"xdr: {xdr}")
...
async def check_exchange():
    ...
    # get offers
    offers_eurmtl_xlm = get_offers(public_exchange_eurmtl_xlm)
    ...
    await update_offer(account_key=public_exchange_eurmtl_xlm, price_min=5, price_max=15,
                       price=cost_eurmtl * persent_eurmtl,
                       selling_asset=eurmtl_asset, buying_asset=xlm_asset, amount=sum_eurmtl_xlm,
                       check_persent=persent_cost,
                       record=offers_eurmtl_xlm.get('EURMTL-XLM'))

    await update_offer(account_key=public_exchange_eurmtl_xlm, price_min=1 / 15, price_max=1 / 5,
    ...
    # btc
    await update_offer(account_key=public_exchange_eurmtl_btc, price_min=15000, price_max=30000,
                       price=round(cost_btc * persent_btc),
                       selling_asset=btcmtl_asset, buying_asset=eurmtl_asset, amount=sum_btcmtl,
                       check_persent=persent_btc_cost, record=offers_eurmtl_btc.get('BTCMTL-EURMTL'))

    await update_offer(account_key=public_exchange_eurmtl_btc, price_min=1 / 30000, price_max=1 / 15000,
                       price=round((1 / cost_btc) * persent_btc, 7),
                       selling_asset=eurmtl_asset, buying_asset=btcmtl_asset, amount=sum_eurmtl_btc,
                       check_persent=persent_btc_cost, record=offers_eurmtl_btc.get('EURMTL-BTCMTL'))

    # sats
    await update_offer(account_key=public_exchange_eurmtl_sats, price_min=15000 / sats_cost,
                       price_max=30000 / sats_cost,
                       price=round(cost_btc * persent_btc / sats_cost, 8),
                       selling_asset=satsmtl_asset, buying_asset=eurmtl_asset, amount=sum_satsmtl_eur,
                       check_persent=persent_btc_cost, record=offers_eurmtl_sats.get('SATSMTL-EURMTL'))

    await update_offer(account_key=public_exchange_eurmtl_sats, price_min=1 / 30000 * sats_cost,
    ...
    await update_offer(account_key=public_exchange_eurmtl_usdc, price_min=0.8, price_max=1.3,
                       ...
                       selling_asset=eurmtl_asset, buying_asset=usdc_asset, amount=sum_eurmtl_usdc,
                       check_persent=persent_cost,
                       record=offers_eurmtl_usdc.get('EURMTL-USDC'))
    await update_offer(account_key=public_exchange_eurmtl_usdc, price_min=0.8, price_max=1.3,
                       price=round(eur_cost * persent_usdc, 4),
                       selling_asset=usdc_asset, buying_asset=eurmtl_asset, amount=sum_usdc, check_persent=persent_cost,
...
async def check_fire(cost_fire):
    ...
    # get balance
    balances = await get_balances(public_fire)
    ...
    await update_offer(account_key=account_fire.account, price_min=0.3, price_max=1, price=round(1 / cost_fire, 5),
                       selling_asset=eurmtl_asset, buying_asset=mtl_asset, amount=sum_eurmtl, check_persent=1.01,
...
if __name__ == "__main__":
    ...
    if welcome_message.get('stop_exchange'):
        exit()
