...
max_eurmtl = 200.0 #max offer
max_xlm = 203.0

min_xlm = 3.0   
persent_eurmtl = 1.0  #0% наценки 
persent_xlm = 1.02  #2% наценки 
...
if ((offer_eurmtl_id == 0) or (cost*persent_eurmtl > eurmtl_sale_price*persent_cost) or (cost*persent_eurmtl*persent_cost < eurmtl_sale_price) or (sum_eurmtl != eurmtl_sale_sum)): 
    if (cost < min_price) or (cost > max_price) or (sum_eurmtl < 1):
        pass
    else:
        ...
        transaction.append_manage_sell_offer_op(selling=asset_eurmtl, buying=asset_xlm, amount=str(sum_eurmtl), 
                                                price=Price.from_raw_price(str(round(cost*persent_eurmtl,7))),offer_id=offer_eurmtl_id)
...
if ((offer_xlm_id == 0) or (1/cost*persent_xlm > xlm_sale_price*persent_cost) or (1/cost*persent_xlm*persent_cost < xlm_sale_price) or (sum_xlm != xlm_sale_sum)): 
    if (cost < min_price) or (cost > max_price) or (sum_xlm < 1):
        pass
    else:
        ...
        transaction.append_manage_sell_offer_op(selling=asset_xlm, buying=asset_eurmtl, amount=str(sum_xlm), 
                                                price=Price.from_raw_price(str(round((1/cost)*persent_xlm,7))),offer_id=offer_xlm_id)
        transaction = transaction.build()
        transaction.sign(private_exchange)
        xdr = transaction.to_xdr()
        logger.info(f"xdr: {xdr}")
